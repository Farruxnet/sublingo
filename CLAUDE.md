...
**v0.9 changes:** Replaced the ad-hoc back controls on study pages with a consistent `.study-appbar` — a slim sticky bar (52px, frosted-glass surface, `top: 0`) present on `words.html`, `flashcards.html`, and `test.html`. Layout: left = chevron-left back icon (44×44px tap target) → `deck.html`; center = deck name (Fraunces, truncated with ellipsis); right = empty. `.study-tabs` updated to pure mode-switcher (Words / Flashcards / Test) with `← Deck` tab removed; its `top` offset corrected from `57px` to `52px` to stack flush below the appbar. Removed the inline `← [deck name]` links that were inside `#studyScreen` (flashcards) and `#questionScreen` (test). `deckLabel` element and references removed from both pages; `flashcards.js` and `test.js` now write deck name to `#studyAppbarTitle` instead.
